...
//    Set<UsersWithRole> getUsersWithRole();
	@Query(nativeQuery=true)
	Optional<Users> loginAll(@Param("Email") String email,@Param("Password") String password);
	
	@Query(nativeQuery=true)
	Optional<Users> loginbyEmail(@Param("Email") String email);

	@Query(nativeQuery=true)
    ArrayList<PermissionResult> getPermissionsForUser(@Param("user_id") long user_id);

	void save(Optional<Users> user1);
}
